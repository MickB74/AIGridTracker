...
from src.constants import (
    GOOGLE_NEWS_RSS, STORY_QUERY, STORY_QUERY_ACTIONS, STORY_ANGLES,
...
def _fetch_story_items(query: str, max_age_days: int):
    """One RSS pull for the community-stories feed. Raises on network/parse
    failure — callers pool multiple queries and only error if all fail."""
    url = GOOGLE_NEWS_RSS.format(q=urllib.parse.quote(query))
    r = requests.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
    r.raise_for_status()
    root = ET.fromstring(r.content)
...
def fetch_community_stories(limit: int = 12, max_age_days: int = 7):
    ...
    under-construction data center: the resident-impact query pooled with the
    bans/lawsuits/legislation query, deduped by link. Filters to the last
    `max_age_days` by pubDate. Returns (list_of_dicts, error_or_None); each
    dict has title/source/link/published/age_days."""
    seen, out, errors = set(), [], []
    for query in (STORY_QUERY, STORY_QUERY_ACTIONS):
        try:
            items = _fetch_story_items(query, max_age_days)
        except Exception as e:                                    # noqa: BLE001
            errors.append(str(e))
            continue
        for it in items:
